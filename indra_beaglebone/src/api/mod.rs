                ...
                let handle = async move {
                    if let Some(db) = POOL.get() {
                        if let Ok(rows) = db.process_request(params).await {
                            log::info!("GetRecords => Client, {} rows returned", rows.len());
...
pub async fn run(events_tx: EventsTx, mode_tx: ChademoTx) -> Result<(), Error> {
    ...
    let try_socket = TcpListener::bind(&addr).await;
    let listener = try_socket.expect("Failed to bind");
    info!("WebSocket API listening on: {}", addr);
    while let Ok((stream, _)) = listener.accept().await {
        let etx = events_tx.clone();
        let mtx = mode_tx.clone();
        let _handle = tokio::spawn(accept_connection(stream, etx, mtx));
...
async fn accept_connection(stream: TcpStream, events_tx: EventsTx, mode_tx: ChademoTx) {
    ...
    let process_incoming_ws = |msg: Message| -> Result<Message, tungstenite::Error> {
        let cmd = match msg {
            Message::Text(cmd) => {
                info!("Text: {:?}", cmd);
                cmd
    ...
    let result = read
        .try_filter(|msg| future::ready(msg.is_text() || msg.is_binary()))
        .map_ok(|s| process_incoming_ws(s))
        .forward(write)
        .await;
    if let Err(e) = result {
        eprintln!("ws error {e:?}")
